...
def battle(monster):
    ...
    if statDictionary["Player Health"] <= 0:
        check_win = False
    else:
        check_win == True
    statDictionary["Player Health"] = original_player_health
    statDictionary[f"{monster} Health"] = original_monster_health
    win_or_lose(check_win)
...
while True:
    ...
    if play_or_exit == 1:
        ...
        if escape_choice == 1:
            ...
            steal_keys = input("Do you want to try to steal the guards keys to get out of the kingdom slightly faster(Y\N)?")
            steal_keys.upper()
            if steal_keys == "Y":
                battle("Guard")
                statDictionary["Player Health"] += 10
            ...
            wins = 2
            time.sleep(2)
            bright_or_dark = input("1. Go through the brightly lit, guarded area thst goes straight to the gates\n2. Go through the dark alley that smells funny\nInput the number for the path you wish to choose: ")
            num = bool(bright_or_dark.isdigit())
            if num == True:
                continue
            else:
                while num == False:
            ...
            if bright_or_dark == 1:
                ...
                if stay == "Y":
                    ...
                    time.sleep(1)
                    stab = input("1. Pour boiling water on the witch\n2. Stab the witch with a knife\nInput the number for the option you wish to choose")
                    num = bool(stab.isdigit())
                    if num == True:
                        continue
                    else:
                        while num == False:
